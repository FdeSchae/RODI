...
After recording hardware triggered, synchronized images to binary files in the previous RECtoBIN script,
this BINtoAVI file converts the binary file back to a vector of images and creates a video file as AVI.
Note that the FrameRate, imageHeight and imageWidth, color and compression settings relative to the previous
recording has to be changed in the respective metadata file.
...
// Initialize Config parameters with standards, will be updated by metadata config file
double frameRateToSet = 100;
int imageHeight = 1080;
int imageWidth = 1440;
int color = 1; // 1= color, else = mono
std::string chosenVideoType = "0";
int h264bitrate = 1000000; // 1000000 - 16000000
int mjpgquality = 90; //1-100
int maxVideoSize = 4; // max file size in GB, 0 indicates no limit (not recommended).
int maxRAM = 10; // max RAM in GB to store frames in working memory
std::string path;
/*
================
...
auto readconfig(string metadata)
{
	...
	cout << "\nFramerate=" << frameRateToSet << " fps";
	std::cout << "\nImageHeight=" << imageHeight << " px";
	std::cout << "\nImageWidth=" << imageWidth << " px";
	std::cout << "\nColorVideo=" << color << " (yes=1/no=0)";
	std::cout << "\nchosenVideoType=" << chosenVideoType;
	std::cout << "\nh264bitrate=" << h264bitrate << " bps";
	std::cout << "\nmjpgquality=" << mjpgquality << " %";
	std::cout << "\nmaxVideoSize=" << maxVideoSize << " GB";
	std::cout << "\nmaxRAM=" << maxRAM << " GB";
	std::cout << "\nVideoPath=" << path << endl << endl;

	return result, frameRateToSet, imageHeight, imageWidth, color, chosenVideoType, h264bitrate, mjpgquality, maxVideoSize, maxRAM, path;
}
	...
	try {
		...
		for (int fileCnt = 0; fileCnt < numFiles; fileCnt++)
		{
			...
			while (rawFile.good())
			{
				...
				if (frameCnt > maxRAM / 0.002) // Convert GB to number of frames assuming 2MB per frame
				{
...
$$$$$$$/  $$$$$$$ $$/   $$/ $$$$$$$/$$/      $$$$$$$$/$$$$$$/$$/   $$/ 
	...
	//string metadata;
	//cout << endl << "Enter METADATA file of the specific recording to convert: " << endl;
	...
	string S, T;
	cout << endl << "Enter BINARY files to convert separated by a + sign: " << endl;
